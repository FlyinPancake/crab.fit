...
const GoogleCalendar = ({ timezone, timeStart, timeEnd, times, onImport }: GoogleCalendarProps) => {
  ...
  const epochTimes = useMemo(
    () => times.map((t) => parseSpecificDate(t).epochMilliseconds),
    [times],
  );
  ...
  const importAvailability = useCallback(() => {
    ...
    gapi.client.calendar.freebusy
      ...
      .then(
        (response) => {
          const availabilities = response.result.calendars
            ? Object.values(response.result.calendars).flatMap((cal) =>
                cal.busy.map((a) => ({
      ...
      {calendars && (
        <div className={styles.wrapper}>
          <p className={styles.title}>
            ...
            <button className={styles.linkButton} type="button" onClick={() => setCanLoad(false)}>
              {t("you.integration.logout")}
            </button>
            )
          </p>
          ...
          {calendars.map((calendar) => (
            <div key={calendar.id} className={styles.item}>
              <input
                type="checkbox"
                id={calendar.id}
                ...
                onChange={() =>
                  setCalendars(
                    calendars.map((c) =>
                      c.id === calendar.id ? { ...c, isChecked: !c.isChecked } : c,
                    ),
                  )
                }
              />
              <label className={styles.name} htmlFor={calendar.id} title={calendar.description}>
