            ...
            <div className={viewerStyles.timeLabels}>
              {table?.rows.map((row, i) => (
                <div className={viewerStyles.timeSpace} key={i}>
                  {row && <label className={viewerStyles.timeLabel}>{row.label}</label>}
                </div>
              )) ?? null}
            </div>
                ...
                {column ? (
                  <div className={viewerStyles.dateColumn}>
                    {column.header.dateLabel && (
                      <label className={viewerStyles.dateLabel}>{column.header.dateLabel}</label>
                    )}
                    ...
                    >
                      {column.cells.map((cell, y) => {
                        ...
                        const isSelected =
                          (!(mode.current === "remove" && selecting.includes(cell.serialized)) &&
                            ...
                            style={
                              {
                                ...
                                "--hover-color": isSelected
                                  ...
                                  : palette[1].highlight,
                                ...(cell.minute !== 0 &&
                                  cell.minute !== 30 && { borderTopColor: "transparent" }),
                              ...
                              document.addEventListener(
                                ...
                                () => {
                                  if (mode.current === "add") {
                                    ...
                                    onChange(
                                      value.filter((t) => !selectingRef.current.includes(t)),
                                    );
                                  }
                                  setSelecting([]);
