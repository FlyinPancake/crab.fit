...
const Weekdays = ({ value, onChange }: WeekdaysProps) => {
  ...
  const weekdays = useMemo(
    () =>
      rotateArray(
        range(1, 7).map((i) =>
          Temporal.Now.plainDateISO().add({ days: i - Temporal.Now.plainDateISO().dayOfWeek }),
        ),
        weekStart ? 0 : 1,
      ),
    [weekStart],
      ...
      {weekdays.map((day, i) => (
        <button
          ...
          className={makeClass(
            ...
            ((!(mode.current === "remove" && selecting.includes(day.dayOfWeek.toString())) &&
          ...
          key={day.toString()}
          title={
            day.equals(Temporal.Now.plainDateISO()) ? t("form.dates.tooltips.today") : undefined
          }
          onKeyDown={(e) => {
