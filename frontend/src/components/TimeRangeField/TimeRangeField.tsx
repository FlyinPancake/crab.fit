  ...
  return (
    <Wrapper>
      ...
      <div className={styles.range}>
        <Selection start={value.start} end={value.start > value.end ? 24 : value.end} />
        {value.start > value.end && (
          <Selection start={value.start > value.end ? 0 : value.start} end={value.end} />
        )}
  ...
  return (
    <div
      ...
      onTouchMove={(e) => {
        ...
        let step = Math.round(
          ((touch.pageX - rangeRect.current.left) / rangeRect.current.width) * 24,
        );
        if (step < 0) step = 0;
        if (step > 24) step = 24;
        step = Math.abs(step);
